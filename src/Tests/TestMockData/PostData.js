...
import Comments from './../../contenedores/Comments/comments.js';
import CommentInput from "./../../contenedores/Comment-Input/comment-input.js"

function PostData ({nombre, fecha, carrera, correo, texto, foto, hora, postingImage, comentarios, id}) {
    ...
    useEffect(() => {
        database.ref('foto_perfil').child('1234').on('value', snapshot =>{
            if(snapshot.val()!=null){
                setFotoActual(snapshot.val().Foto)
            }
        });
        database.ref('usuarios').child('1234').on('value', snapshot =>{
            if(snapshot.val()!=null){
